...
func (k *Kinsumer) dynamoTableReady(name string) error {
	out, err := k.dynamodb.DescribeTable(context.TODO(), &dynamodb.DescribeTableInput{
...
func (k *Kinsumer) dynamoTableExists(name string) bool {
	_, err := k.dynamodb.DescribeTable(context.TODO(), &dynamodb.DescribeTableInput{
		TableName: aws.String(name),
	})
	return err == nil
}
...
func (k *Kinsumer) dynamoCreateTableIfNotExists(name, distKey string) error {
	...
	_, err := k.dynamodb.CreateTable(context.TODO(), &dynamodb.CreateTableInput{
...
func (k *Kinsumer) dynamoDeleteTableIfExists(name string) error {
	...
	_, err := k.dynamodb.DeleteTable(context.TODO(), &dynamodb.DeleteTableInput{
		TableName: aws.String(name),
	})
	if err != nil {
		return err
...
func (k *Kinsumer) kinesisStreamReady() error {
	if k.config.useListShardsForKinesisStreamReady {
		_, err := k.kinesis.ListShards(context.TODO(), &kinesis.ListShardsInput{
	...
	out, err := k.kinesis.DescribeStream(context.TODO(), &kinesis.DescribeStreamInput{
		StreamName: aws.String(k.streamName),
	})
	if err != nil {
